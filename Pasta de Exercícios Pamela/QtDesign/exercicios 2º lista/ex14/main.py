...
class EditorTexto(QMainWindow):
    ...
    def mostrarNumero(self):
        try:
            numero = float(self.ui.lineEdit.text())
            self.lista.append(numero)
            self.ui.lineEdit.clear()
            self.ui.label_3.setText("Adicionado.")
            lista_invertida = self.lista[::-1] 
            self.i += 1  
            self.ui.label_5.setText(str(self.i))
        
            if(len(self.lista) == 10):
                self.ui.label.setText("Beleza?")
                self.ui.label_2.setText(str(lista_invertida)) 
            elif(len(self.lista) > 10):
                self.ui.label_3.setText("NÃO")
                self.ui.label.setText("Pô tu ta me testando")
                self.ui.label_4.setText("Chega né o objetivo do exercício é 10 números se vc quer mais ai já num é comigo")
            

            QTimer.singleShot(1000, self.ui.label_3.clear)
